...
//      System.out.println(foodList.size());
   ...
   private void wormDie() {
       // thread.close();
          ...
          if (x < gameAct.getX() || x > 1050 || y < gameAct.getY()-80 || y > 600) {
         ...
         WormDTO dto = new WormDTO();
           ...
           int y = 0;
            ...
            for(int i = 0; i < foodNum ; i++) {
               ...
               //먹이 x,y 중심좌표
               fx = foodList.get(i).getX() + (10 / 2);    
               fy = foodList.get(i).getY() + (10 / 2);
...
//            ======Thread.sleep 이 6번 실행 되면 먹이를 재생성해라. 스레드타임 * 6 = 먹이생성시간
                if (foodNum < 80) {
                     WormDTO foodDto = new WormDTO();
						/*
						 * int cx = (int)(Math.random()*710-40+1)+40; int cy =
						 * (int)(Math.random()*510-130+1)+130;
						 */
                     int cx = (int)(Math.random()*1060-40+1)+40;
                     int cy = (int)(Math.random()*600-130+1)+130;
                     foodDto.setX(cx);
                     foodDto.setY(cy);
